/**
 * ============================================================
 * MODULE: PBK GENERATOR
 * Tự động chưng cất bài TCCS thành entry PHAN_BAC_KHO có cấu trúc.
 *
 * Workflow:
 *   1. runTccsScrapeDrafts()          → scrape bài TCCS
 *   2. syncTccsApprovedChunksToPinecone() → duyệt chunk lên Pinecone
 *   3. generatePhanBacFromTccs()      → sinh entry PBK từ toàn văn bài
...
function pbkSaveEntries_(entries, articleId, articleTitle, sourceUrl) {
  ...
  entries.forEach((entry, index) => {
    ...
    // Nguồn: embed articleId để check "đã xử lý" sau này
...
/**
 * Lấy tập hợp articleId đã được xử lý (dựa vào cột Nguồn của PHAN_BAC_KHO).
 */
...
/**
 * Đánh dấu bài đã xử lý (không có luận điệu nào) bằng 1 entry placeholder.
 * Tránh bài bị xử lý lại lần sau.
 */
function pbkMarkProcessed_(articleId) {
  ...
  sheet.appendRow([
    ...
    'SKIP',
    'Bài không có luận điệu sai trái cụ thể',
...
/**
 * Đọc metadata (title, topic) từ TCCS_ARTICLES theo articleId.
 */
