...
- services/thu-tuan/Code.gs độc lập, project không public: trigger → script lock → duyệt/dấu HMAC (khóa THU_TUAN_APPROVAL_SECRET trong Script Properties) + allowlist người duyệt → Sheets riêng → MailApp từng người → nhật ký khóa kỳ/mã (khớp theo Khoa, cột text). Không doGet/doPost, không cùng backend public. Email HTML responsive + plain-text theo bố cục đã định; dữ liệu Sheet được escape, runtime không gọi AI; NotebookLM tùy chọn, chỉ là link tra cứu cuối thư kèm nhắc đối chiếu nguồn. services/thu-tuan/tools/corpus-to-sheet.cjs chuyển bộ Markdown 52 tuần (ngoài repo) sang CSV nháp, không đánh dấu duyệt. Test: tests/thu-tuan.test.cjs.
